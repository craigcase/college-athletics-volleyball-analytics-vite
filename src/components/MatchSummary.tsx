...
function RallyMetric({metric,subject,rows}:{metric:string;subject:'our_team'|'opponent';rows:MatchSummaryMetric[]}){
  const row=rows.find(item=>item.subject===subject&&item.metric===metric);
  return <div className="rally-metric-side">
    <small>{subject==='our_team'?'US':'OPP'}</small>
...
function RallyAnalytics({summary}:{summary:MatchSummaryData}){
  ...
  return <section className="rally-analytics panel">
    ...
    <div className="rally-metric-grid">
      {rallyOrder.map(metric=><article className="rally-metric-card" key={metric}>
        ...
        {metric==='longest_service_run'?<div className="rally-metric-sides">
          {(['our_team','opponent'] as const).map(side=>{const run=bestRun(side);return <div className="rally-metric-side" key={side}><small>{side==='our_team'?'US':'OPP'}</small>{run?<><strong>{run.pointsWon}</strong><span>{run.serveAttempts} serve attempts · Set {run.setNumber}</span></>:<span className="rally-unavailable">Not available from this evidence</span>}</div>})}
        </div>:<div className="rally-metric-sides"><RallyMetric metric={metric} subject="our_team" rows={analytics.metrics}/><RallyMetric metric={metric} subject="opponent" rows={analytics.metrics}/></div>}
      </article>)}
    </div>
    <div className="sideout-pathways">
      ...
      <div className="pathway-grid">{(['our_team','opponent'] as const).map(side=><div key={side}><strong>{side==='our_team'?'Us':'Opponent'}</strong><span>First-ball sideout <b>{analytics.sideoutPathways[side].first_ball_sideout}</b></span><span>Regular sideout <b>{analytics.sideoutPathways[side].regular_sideout}</b></span><span>Phase unknown <b>{analytics.sideoutPathways[side].unknown_phase}</b></span></div>)}</div>
    </div>
...
export function MatchSummary({summary}:{summary:MatchSummaryData}){
  ...
  return <>
    ...
    {ribbon.length?<section className="stat-ribbon">{ribbon.map(metric=><div className="stat-cell" key={metric}><span>{labels[metric]||metric}</span><div><strong>{by('our_team',metric)?display(metric,by('our_team',metric)!.value):'—'}</strong><small>US</small></div><div><strong>{by('opponent',metric)?display(metric,by('opponent',metric)!.value):'—'}</strong><small>OPP</small></div></div>)}</section>:<section className="empty-panel"><h3>No supported match metrics yet</h3><p>Add a public box score or structured match file. Missing evidence stays missing.</p></section>}
    <RallyAnalytics summary={summary}/>
    <div className="impact-grid"><Impact title="Our Match Impact" rows={our}/><Impact title="Opponent Match Impact" rows={opp}/></div>
...
function Impact({title,rows}:{title:string;rows:any[]}){
